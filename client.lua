...
Citizen.CreateThread(function()
    while true do
        ...
        if GetPedInVehicleSeat(GetVehiclePedIsIn(ESX.PlayerData.ped), -1) == ESX.PlayerData.ped and GetIsVehicleEngineRunning(GetVehiclePedIsIn(ESX.PlayerData.ped)) == 1 then
...
AddEventHandler("rv-status:onTick", function(status)
	TriggerEvent('rv-status:getStatus', 'hunger', function(status)
		hunger = status.val / 10000
	end)
	TriggerEvent('rv-status:getStatus', 'thirst', function(status)
